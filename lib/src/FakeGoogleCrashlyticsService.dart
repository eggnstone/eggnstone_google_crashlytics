...
class FakeGoogleCrashlyticsService
    implements IGoogleCrashlyticsService
{
    ...
    FakeGoogleCrashlyticsService._internal(this._fakeFirebaseCrashlytics, this._additionalCrashReporterCallback, bool isEnabled, bool isDebugEnabled)
    ...
    static Future<IGoogleCrashlyticsService> create(CrashReporterCallback? alternativeCrashReporterCallback, IAnalyticsService? analyticsService, bool startEnabled, bool startDebugEnabled)
    {
        ...
        return FakeGoogleCrashlyticsService.createMockable(fakeFirebaseCrashlytics, alternativeCrashReporterCallback, startEnabled, startDebugEnabled);
    }
    ...
    async
    {
        final FakeGoogleCrashlyticsService instance = FakeGoogleCrashlyticsService._internal(crashlytics, alternativeCrashReporterCallback, startEnabled, startDebugEnabled);
        //instance._init();
        return instance;
    }
    ...
    {
        logError('##################################################');
        logError('# FakeGoogleCrashlyticsService.onError');
        ...
        if (_isEnabled)
        {
            try
            {
                ...
                logInfo('# FakeGoogleCrashlyticsService.onError/_fakeFirebaseCrashlytics.recordError succeeded.');
            }
            catch (e2, stackTrace2)
            {
                logError('##################################################');
                logError('# FakeGoogleCrashlyticsService.onError/_fakeFirebaseCrashlytics.recordError failed!');
            ...
            if (_additionalCrashReporterCallback != null)
            {
                final Map<String, Object> map =
                    <String, Object>
                    {
                        ...
                        'CrashlyticsSource': 'FakeGoogleCrashlyticsService.onError'
                    };
                ...
                try
                {
                    ...
                    logInfo('# FakeGoogleCrashlyticsService.onError/_additionalCrashReporterCallback succeeded.');
                }
                catch (e2, stackTrace2)
                {
                    logError('##################################################');
                    logError('# FakeGoogleCrashlyticsService.onError/_additionalCrashReporterCallback failed!');
                    logError(e2.toString());
                    logError(stackTrace2.toString());
                    logError('##################################################');
                }
